\documentclass{tfgitic}[2025/09/29]
...
\title{
  Com escriure la memòria del TFE
}

\subtitle{
  Manual de composició, ortotipografia i tipografia\\
  Versió~\classversion{}
}
...
Aquest estil està especialment dissenyat per a facilitar l'escriptura de
la memòria del treball final d'estudis, \acro{tfe}. En els seus
orígens va ser dissenyada per al Grau d'Enginyeria de Sistemes TIC a
UPC Manresa però, especialment a partir de la versió 7.0, l'estil s'ha
generalitzat per a poder ser usat en qualsevol dels estudis de grau i
màster d'UPC Manresa. L'estil respecta i implementa fil per randa els
requeriments de la normativa actual,
\cite{manresa23:_normat_trebal_fi_estud_epsem}.

Al ser un estil pel \LaTeX{}, s'assumeix que teniu coneixements
d'aquesta eina. En el cas que no l'hagueu provat mai, lluny de
desanimar-vos us convidem a fer-ho!  És una eina molt interessant de
la que podreu gaudir molt temps. També s'assumeix que la plataforma
sobre la que treballeu és Linux. En cas que no conegueu \LaTeX{} us
aconsellem que feu un cop d'ull a l'apèndix~\ref{ape:intro-latex}.
...
\fitx{docum-tfe.tex}, \fitx{docum-tfe.bib},,
\fitx{taula-exemple-1.png}, \fitx{taula-exemple-2.png},
\fitx{taula-exemple-3.png}, \fitx{taula-exemple-3.png} i
\fitx{halebopp3.jpg}. Si es compilen adequadament generen el fitxer en
format \texttt{pdf} que esteu llegint.
...
\item \emph{Valors numèrics}. Els valors numèrics s'escriuen fent
  servir la coma com a separador decimal. Per facilitar com s'escriuen
  els valors en el document fem servir el paquet \texttt{siunitx}
  ---que ja està carregat per omissió. Per exemple escrivim
  \verb!\num{23.036}! per obtenir \num{23.036}; \verb!\ang{23}! per
  obtenir \ang{23}; \verb!\ang{23;10;12}!  per obtenir \ang{23;10;12};
  \verb!\num{30.45d-4}! per obtenir \num{30.45d-4};
  \verb!\num{3.45d-4}!  per obtenir \num{3.45d-4};
  \verb!\complexnum{2+4i}!  per obtenir \complexnum{2+4i};
  \verb|\numlist{12.; 10.091; 11.54}| per obtenir \numlist{12.;
    10.091; 11.54}; \verb|\numproduct{5 x 100 x 2}| per obtenir
  \numproduct{5 x 100 x 2}; \verb|\numrange{5}{100}| per obtenir
  \numrange{5}{100}.
...
\item \emph{Unitats}. Les unitats, sempre del \acro{si}, s'escriuen
  ...
  Per exemple, podem escriure \verb!\qty{10.23}{\kilo\ohm}! per
  obtenir aquest resultat \qty{10.23}{\kilo\ohm};
  \verb!\qty{10.239}{\micro\farad}! per obtenir
  \qty{10.239}{\micro\farad}; o bé
  \verb!\qty{0.23d-3}{\metre\per\second}!  per obtenir
  \qty{0.23d-3}{\metre\per\second} ---noteu la manera simplificada
  d'escriure magnituds en notació científica---;
  \verb|\qty{100}{\degreeCelsius}| per obtenir
  \qty{100}{\degreeCelsius} o
  \verb!\qty{12.012}{\mega\bit\per\second}!  per obtenir
  \qty{12.012}{\mega\bit\per\second};
  \verb|\qtylist{12.; 10.091; 11.54}{\ohm}| per obtenir \qtylist{12.;
    10.091; 11.54}{\ohm}; \verb|\qtyproduct{5 x 100 x 2}{\kilo\meter}|
  per obtenir \qtyproduct{5 x 100 x 2}{\kilo\meter};
  \verb|\qtyrange{5}{100}{\degreeCelsius}| per obtenir
  \qtyrange{5}{100}{\degreeCelsius}.


  Hi ha abreviatures per a algunes unitats habituals que fan més
  còmode escriure-les. Per exemple, \verb|\qty{3.}{\us}| resulta en
  \qty{3.}{\us}; \verb|\qty{2.059}{\kg}| resulta en \qty{2.059}{\kg} i
  \verb|\qty{2.3}{\kHz}| resulta en \qty{2.3}{\kHz}.
  ...
  \qty{47}{\kilo\ohm}». D'altra banda, si el que volem és indicar la
  resistència que hem mesurat en el primari d'un transformador
  escriuríem «la resistència del primari és de \qty{34.123}{\ohm}».
...
\item \emph{Fórmules i elements químics}. Les fórmules químiques cal
  ...
  \ce{CO2 + C -> 2CO} i escrivint
  \verb!\ce{SO4^2- + Ba^2+ -> BaSO4 v}!  obtenim \ce{SO4^2- + Ba^2+ ->
    BaSO4 v}. Si escrivim
  \verb|$\ce{x Na(NH4)HPO4 ->[\Delta] (NaPO3)_x + x NH3 ^ + x H2O}$|
  obtenim
...
que cal tenir en compte. L'apèndix~\ref{ape:disseny-taules}
...
\begin{tabularx}{\textwidth}{lX}
  ...
  %current version
  8.2 & Algunes errates.\\
  8.1 & Afegits números d'apartat als bookmarks del pdf.
  Millores al manual. Correcció errata caçada per Pau de las Heras.
  Títol més descriptiu al manual.
  Nova macro \verb/\classversion/.\\
  8.0 & Afegida documentació en anglès, \fitx{docum-tfe-en}.
  Afegit un annex sobre composició de taules.
  Actualització i localització de \texttt{siunitx}.
  Millora del resum.\\
  7.1 & Actualització de les versions mínimes d'alguns
  paquets. Millores en la i18n.  Afegit gènere/número a les macros
  sobre persones. Millores en la partició de mots en català. Missatges
  d'error per a combinacions impròpies de metadades.  Reflecteix
  informació tipogràfica a la cara parell de la portada.
...
\chapter{Com compondre taules}
\label{ape:disseny-taules}

Les taules mal dissenyades són una mancança habitual en els manuscrits
del \acro{tfe}. Aquest apèndix, seguint a Bringhurst i Tufte, presenta
recomanacions sobre com dissenyar taules efectives i mostra com
implementar aquestes idees en \LaTeX{}.

També es presenten exemples reals extrets de diversos \acro{tfe}: cada
taula original s’analitza, es redissenya i es compon novament.

\section{Bones pràctiques per al disseny de taules}
\label{sec:good-pract-tables}

Bringhurst \cite[70]{bringhurst04:_elemen_typog_style} dedica diverses
pàgines a la tipografia tabular. Les transparències de Püschel
\cite{püschel10:_small_guide_makin_nice_tables} en donen un resum
concís orientat a \LaTeX{}. El disseny de taules és exigent; cal
abordar-lo deliberadament. Cal tenir en compte els principis següents:

\begin{enumerate}

\item \emph{Propòsit i necessitat}\par
  Feu servir una taula només quan l’estructura millora realment la
  comprensió. Si la informació es pot llegir igual de bé en format
  lineal, es millor la prosa o una llista.

  Cada taula ha de respondre a una pregunta clara. Suprimiu qualsevol
  columna, fila o xifra que no hi contribueixi.

\item \emph{Economia i simplicitat}\par
  Mostreu les dades, no la decoració. Minimitzeu els
  filets\footnote{Un filet és una ratlla usada en tipografia per
    separar o remacar (en anglès \est{rule})}, caixes, ombrejats i
  ornaments.

  Prioritzeu l’espai en blanc en comptes dels filets. L’espaiat entre
  columnes o grups de columnes sol substituir completament els filets
  verticals.

  Eviteu el «soroll visual»: degradats, farciments pesats, negretes
  gratuïtes, unitats repetides a cada ce\l.la i decimals innecessaris.

\item \emph{Jerarquia i èmfasi}\par
  Construiu la jerarquia amb la posició, el pes i l'espaiat ---no amb
  la decoració. L’ull ha de localitzar primer la capçalera i després
  els agrupaments lògics.

  Els encapçalaments poden fer servir un estil lleugerament diferent
  (versaletes, espaiat subtil entre lletres, canvi lleu de pes), però
  no han de dominar el cos.

  Les files amb totals o resums es poden separar amb un filet fi a
  dalt, espai addicional o un petit canvi tipogràfic ---reserveu els
  excessos (majúscules, negreta gruixuda) per a casos d’emergència.

\item \emph{Filets}\par
  Feu servir el mínim de filets horitzontals: sovint només (a) un
  filet dalt de tot, (b) un separador entre capçalera i cos, (c) un
  filet opcional per a subtotals o totals, i (d) un filet per acabar a
  sota.

  Eviteu els filets verticals; l’alineació i l’espaiat han de fer la
  feina.

  Feu servir filets molt fins. Els filets gruixuts dominen massa i
  trenquen el ritme de la pàgina.

\item \emph{Alineació i xifres}\par
  Alineeu els números segons el seu significat:
  \begin{itemize}
  \item Enters, alineats a la dreta.
  \item Decimals, alineats al separador decimal (useu la columna
    \texttt{S} de \fitx{siunitx} o \fitx{dcolumn}).
  \item Escriviu les unitats a la capçalera o en una columna separada
    ---no les repetiu a cada ce\l.la.
  \end{itemize}

  % Utilitza xifres tabulars (lining) per a columnes numèriques; les
  % xifres proporcionals són adequades per a prosa.

  Useu el signe menys per als nombres negatius; reserveu els guions
  mitjans per a intervals. Consulteu el punt sobre guions a la
  Secció~\ref{cap:ortotipografia}.

\item \emph{Consistència i precisió} \par
  Arrodoniu de manera consistent. No suggeriu una precisió que les
  dades no tenen.

  Ordeneu les files i columnes de manera lògica (cronologia, magnitud,
  categoria). Si el principi d’ordenació no és evident, expliqueu-vos.

\item \emph{Espaiat i proporció}\par
  Alçada de les files: useu prou interlineat per a una lectura còmoda;
  eviteu tant l’amuntegament com els espais massa grans.

  Separació entre columnes: manteniu l’espai mínim necessari; un espai
  lleugerament més ample pot indicar un límit de grup.

  Manteniu les taules dins l’amplada del text principal tret que sigui
  imprescindible fer-les més amples; les taules molt amples dificulten
  la lectura.

\item \emph{Tipografia dins de les ce\l.les}\par
  Si la taula és densa, reduïu lleugerament el cos de lletra
  (\qtyrange{0.5}{1}{pt}), però mai tant que cansi la vista.

  Eviteu èmfasitzats que competeixin entre ells. Reserveu la negreta o
  la cursiva per a un sol nivell semàntic, mai tots dos alhora.

  % Utilitza versaletes amb moderació (p. ex. per a abreviacions en
  % capçaleres) per mantenir una textura calmada.

\item \emph{Agrupament i flux de lectura}\par
  Agrupeu files relacionades amb l'espai en blanc, no amb filets. Un
  petit interlineat extra («air line») sovint és suficient.

  Alineeu el text de les columnes a l’esquerra; centreu només
  etiquetes molt curtes i homogènies.

  Eviteu que les etiquetes clau es trenquin en dues línies. Si una és
  massa llarga, escurceu-la i expliqueu-ho en una nota.

\item \emph{Notes, fonts i comentaris}\par
  Anoteu amb mesura. Feu servir marques superiors o símbols i reculliu
  les explicacions sota la taula en cos menor.

  Les llegendes han de ser concises i específiques.

\item \emph{Accessibilitat i claredat}\par
  No us baseu només en el color; la diferenciació ha de ser clara
  també en escala de grisos.

  Expliqueu les abreviacions no òbvies a l’encapçalament o en una
  nota.

  Assegureu l’escaneig visual: el lector hauria de copsar el
  significat de cada columna i localitzar els totals en
  \qtyrange{2}{3}{\second}.

\item \emph{Eviteu el sobre-disseny}\par
  No encaselleu cada ce\l.la. Les graelles empresonen les dades; els
  dissenys oberts afavoreixen la comparació.

  Feu servir ombrejats només si la jerarquia no és prou clara sense
  ells. Si cal, que siguin molt lleugers i consistents.
...
\section{Pistes per a la implementació en \LaTeX{}}

\begin{itemize}
\item Carregueu el paquet \texttt{booktabs}: \verb|\toprule|,
  \verb|\midrule|, \verb|\addlinespace|, \verb|\bottomrule|; eviteu
  els filets verticals.
\item Useu les columnes \verb|S| del paquet \texttt{siunitx} per a
  l’alineació numèrica, l’arrodoniment i l’alineació decimal.
\item Poseu les unitats a l’encapçalament de columna,
  p. ex. \verb|\multicolumn{1}{c}{Voltage (\unit{\volt})}|.
\item Separeu els grups lògics de files amb \verb|\addlinespace| en
  comptes de múltiples \verb|\midrule|.
\item Mantingueu les llegendes breus, per ex.:\par
  \verb|\caption{Latency benchmarks for API endpoints ($n=500$).}|
\item Reviseu sempre una còpia impresa de la taula; ajusteu l’espaiat
  i el gruix dels filets després de veure’n el resultat físic.
\end{itemize}



\section{Exemples de taules}
...
\begin{figure}
  ...
  \caption{Taula original de l’exemple 1}
  \label{fig:source-table-1}
\end{figure}

En aquest exemple s'analitza la taula de la
Figura~\ref{fig:source-table-1}, extreta d’un treball fi de
màster. Diversos aspectes contradiuen les bones pràctiques de la
Secció~\ref{sec:good-pract-tables}.
\begin{itemize}
\item La decoració (franges de color, etc.) no aporta informació i
  s’hauria de suprimir.
\item Es fa servir negreta tot i que cal reservar-la per a
  l’estructura.
\item L’alineació centrada és inadequada per a columnes numèriques.
\item Els encapçalaments són massa llargs.
\item Les columnes podrien jerarquitzar-se per facilitar la lectura.
\end{itemize}
...
El redisseny de la Taula~\ref{tab:redesigned-table-1} corregeix
aquests problemes:
\begin{itemize}
\item S’ha eliminat tota decoració supèrflua.
\item S’han suprimit els filets no essencials. No hi ha filets
  verticals i només hi ha els filets horitzontals estrictament
  necessàries. L’estructura es fa evident mitjançant l’alineació i
  l’espaiat.
\item Les columnes estan alineades segons els estàndards habituals.
\item Les columnes s’han reordenat per permetre agrupacions
  significatives i un encapçalament jeràrquic.
\item La informació temporal repetitiva («any») s’ha traslladat a la
  llegenda.
\item No s’ha fet servir negreta. L’estructura per si sola clarifica
  els encapçalaments; només s’ha destacat el nivell superior per
  mostrar la jerarquia.
\item Els noms de les empreses s’havien anonimitzat a l’original;
  «Consultora 1», etc., només afegeixen amplada. La numeració simple
  és suficient i redueix l’ample.
\end{itemize}

Els encapçalaments són amb lletra dreta i prou. La seva funció es fa
evident gràcies a les línies horitzontals. Només s’hi destaca la capa
jeràrquica. En una taula simple i no jeràrquica, no caldria cap
èmfasi. El resultat és més clar i fàcil de llegir.

Una bona llegenda seria: «Taula 2: Consum anual de paper de tres
consultores, consum induït i impacte.»

Codi font de la taula redissenyada:
\begin{verbatim}
\begin{table}
  \centering
  \sisetup{table-number-alignment = right}
...
\begin{figure}
  ...
  \caption{Taula original de l’exemple 2}
  \label{fig:source-table-2}
\end{figure}

La Figura~\ref{fig:source-table-2} mostra una altra taula
problemàtica. Aquesta compara el cost de dues ofertes de connexió
\acro{nb-iot} d’una mateixa empresa. Els problemes més destacats són:
\begin{itemize}
\item L’excés de filets crea una gàbia.
\item Les columnes estan desalineades.
\item Les unitats es repeteixen a cada ce\l.la.
\item Els encapçalaments en negreta són massa pesants.
\item Els títols de columna són massa llargs i eixamplen
  innecessàriament les columnes.
\item La taula es força a ser en línia, en lloc de flotant. Vegeu
  l'apartat~\ref{sec:taules}.
\item No queda clara la distinció entre costos fixos
  (SIM/configuració) i variables.
\end{itemize}
...
El redisseny es mostra a la Taula~\ref{tab:redesigned-table-2}. Els
encapçalaments s’han escurçat i les unitats s’han desplaçat a la
següent línia per a mantenir una bona separació entre columnes. Ara,
la relació jeràrquica a l’encapçalament és explícita. Una bona
llegenda seria: «Taula 6.1: Preu del servei \acro{nb-iot} de Vodafone,
[IoT20]. El cost fix inclou la \acro{sim} i la taxa de connexió.»
...
\begin{figure}
  ...
  \caption{Taula original de l’exemple 3}
  \label{fig:source-table-3}
\end{figure}

La Figura~\ref{fig:source-table-3} compara diversos mòduls de
comunicació \acro{iot}. Alguns són compatibles amb \acro{nb-iot} i
\acro{lte-m}. Els problemes formals recorden els exemples anteriors:
\begin{itemize}
\item Massa línies.
\item Encapçalaments massa destacats.
\item Les unitats es repeteixen a cada ce\l.la.
\item Les citacions a la primera columna segurament ja apareixen al
  text i es poden ometre.
\end{itemize}

Els problemes de contingut són més seriosos:
\begin{itemize}
\item «dDRX» és opac; algunes ce\l.les contenen dos valors, ells
  mateixos duplicats.
\item «Average (max/min)» és ambigu. Es refereix a la mitjana dels
  màxims/mínims o als extrems de les mitjanes?
\item A la primera fila es distingeix el consum en transmissió i
  recepció; a la resta no. Què representen els valors?
\end{itemize}
...
\begin{table}
  ...
  \begin{tabular}{
    ...
    \bottomrule
  \end{tabular}
  \caption{Redisseny de la taula de la
    Figura~\ref{fig:source-table-3}. Modes de baix consum.}
  \label{tab:low-power}
\end{table}
...
\begin{table}
  ...
  \end{tabular}
  \caption{Redisseny de la taula de la
    Figura~\ref{fig:source-table-3}. Consum Tx/Rx.}
  \label{tab:txrx-power}
\end{table}

Aquí s’ha dividit l’original en dues taules complementàries per
millorar la claredat i evitar una longitud excessiva. La
Taula~\ref{tab:low-power} cobreix el consum en modes de baix consum;
la Taula~\ref{tab:txrx-power} es centra en el consum en
transmissió/rebuda. El codi font de les dues taules és:
...
\begin{figure}[tb]
  ...
  \caption{Taula original de l’exemple 4}
  \label{fig:source-table-4}
\end{figure}

La taula de la Figura~\ref{fig:source-table-4} recull una comparació
no numèrica (programari de realitat virtual). Els seus defectes són
coneguts:
\begin{itemize}
\item De nou, un ús abusiu de línies ---molt comú!
\item S’hi fa servir negreta tot i que ja s’aplica jerarquia
  mitjançant seccions.
\item Diverses ce\l.les són massa verboses.
\end{itemize}
...
\begin{table}[tb]
  ...
  \begin{tabularx}{0.95\linewidth}{@{} l L L L L @{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Redisseny de la taula de la Figura~\ref{fig:source-table-4}}
  \label{tab:soft-comparison}
\end{table}

La Taula~\ref{tab:soft-comparison} presenta el redisseny. Les millores inclouen:
\begin{itemize}
\item Supressió de filets innecessàris.
\item Reducció i normalització de l’èmfasi.
\item Contingut de les ce\l.les escurçat.
\item Files reordenades segons una seqüència més significativa.
\item Jerarquia vertical afegida per facilitar l’escaneig visual.
\item Reducció lleu del cos de lletra per estalviar espai.
\end{itemize}
